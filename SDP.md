...
Для проекта формируется компактная кросс-функциональная команда из 3 человек. Роль в плане не равна человеку: один исполнитель может закрывать несколько ролей, а расчёт стоимости ведётся по фактическим часам в каждой роли. Отдельно разделены **Backend Lead** и **Backend Engineer**: Backend Lead отвечает за техническое лидерство, архитектурные решения и code review, а Backend Engineer — за реализацию серверных задач.
