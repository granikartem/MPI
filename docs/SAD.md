...
«Создание заявки», «Оценка риска», «Хранение данных», «Адаптер Wasteland Intel» и т.д.). Внутри
`application`, `domain` и `infrastructure` компоненты, относящиеся к одному предметному модулю или
одной технической категории, собраны в именованные подпакеты — «Заявка и рейс», «Маршрут и риски»,
«Организации» (по категориям прецедентов из [UseCases.md](UseCases.md)) и «Документные хранилища»,
«Внешние адаптеры» (по технологии хранения и роли адаптера в `infrastructure`); категории, где остался
только один компонент (команда, груз, полевые события, финансы, пользователи, аудит, PostgreSQL,
безопасность), сгруппированы не были — вложенность вводится только там, где она действительно что-то
объединяет. Фактическая структура кода — пакеты по предметным модулям (`com.karavany.request`,
`.organization`, `.route`, `.risk`, `.audit`) со слоями `web` / `service` / `domain` / `repository`
внутри; соответствие приведено заметкой на диаграмме.
...
на :443, проксируя `/api/*` на backend; OpenJDK 21 исполняет `karavany-0.1.0.jar` — исполняемый fat-jar
Spring Boot 3.3.5 со встроенным Apache Tomcat 10.1.31 (Jakarta Servlet 6.0, HTTP/1.1 NIO-коннектор на
127.0.0.1:8080, пул 10–200 потоков): запросы принимает Tomcat-коннектор, маршрутизирует DispatcherServlet
Spring MVC, данные читаются и пишутся через Spring Data JPA (PostgreSQL) и Spring Data MongoDB, схема
БД накатывается Flyway при старте, состояние отдаётся Spring Boot Actuator (`/api/health`, `/api/info`).
Отдельный WAR и внешний сервер приложений не используются — Tomcat разворачивается вместе с приложением
одной командой `java -jar`. PostgreSQL 16 и MongoDB 7.0 хранят данные на отдельных ZFS-датасетах. Backend
и базы данных слушают только `127.0.0.1`, извне сетевой фильтр pf пропускает только TCP 443. Резервные
копии (`pg_dump`, `mongodump`, ZFS-снапшоты) записываются на голотейп. Клиенты — до 50 офисных терминалов
