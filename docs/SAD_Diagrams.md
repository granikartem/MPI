...
- **Подпакеты внутри слоёв.** `presentation` и так был разбит на три под-пакета (Web-клиент, PWA,
  REST API), а `application`, `domain` и `infrastructure` до правки были плоскими списками. Там, где
  у слоя нашлось два и более компонента одного предметного модуля или технической категории, они
  собраны в под-пакет: «Заявка и рейс», «Маршрут и риски» — в `application` и `domain` (по категориям
  прецедентов [UseCases.md](UseCases.md)), «Организации» — в `application`, «Документные хранилища»
  (MongoDB) и «Внешние адаптеры» (REST) — в `infrastructure`. Категории с ровно одним компонентом
  (команда, груз, полевые события, финансы, пользователи, аудит, PostgreSQL, безопасность) оставлены
  отдельными компонентами без обёртки — под-пакет из одного элемента ничего не группирует, только
  добавляет вложенность. Имена компонентов и связи между пакетами не менялись.
...
- **Внутри JVM — встроенный Tomcat.** `karavany-0.1.0.jar` детализирован отдельным узлом: Apache
  Tomcat 10.1.31 (Jakarta Servlet 6.0), встроенный в jar через `spring-boot-starter-tomcat`, — его
  HTTP/1.1 NIO-коннектор на 127.0.0.1:8080 с пулом 10–200 потоков принимает запросы от nginx, а
  дальше DispatcherServlet Spring MVC маршрутизирует их в контроллеры; исходящие связи к PostgreSQL
  (JDBC), MongoDB (Wire Protocol), Wasteland Intel и NCR Checkpoint (REST/HTTPS) идут уже от артефакта
  приложения, а не от коннектора. Отдельного WAR и внешнего сервера приложений в проекте нет —
  показано именно то, что реально собирает `gradle bootJar` и запускает `java -jar`.
